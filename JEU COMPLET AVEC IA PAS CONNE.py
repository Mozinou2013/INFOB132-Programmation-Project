...
def AI_final(player_A, player_B, food, height, width):
    ...
    Versions:
    ---------
    specificaition: Eline Mota, Louise Delpierre, Aline Boulanger, Aurélie Genot (v.1 21/04/2022)
    ...
    if x_A_O > 3 or y_A_O > 3:
        ...
        tuple0 =  move_wolf_to_wolf(pos_alpha_A, pos_omega_A, orders)#rapproche l'alpha de l'omega
        orders = tuple0[0]
        pos_alpha_AB  = tuple0[1]
        tuple1=  move_wolf_to_wolf(pos_omega_A, pos_alpha_AB, orders)#rapproche l'omega de la nouvelle position de alpha
        orders = tuple1[0]
        ...
        for wolves in player_B:
            ...
            if x_A_W <= 2 and y_A_W <= 2: #si des méchants loups sont trop proches
                ici = 'oui'
                ...
                if player_A[pos_omega_A]['life'] >= 40:
                    ...
                    for foods in food:
                        ...
                        distance_xO = distance_O[0]
                        distance_yO = distance_O[1]
                        distance_xA = distance_A[0]
                        distance_yA = distance_A[1]

                        if distance_xO <= 1 and distance_yO <=1: 
                            orders = IA_game_eat(pos_omega_A, foods, orders) #omega mange
                            if distance_xA <= 1 and distance_yA <= 1:
                                orders = IA_game_eat(pos_alpha_A, foods, orders ) #alpha mange

                        
                                
        
                    hungry0 = move_food(pos_omega_A, food, orders) #si pas de nourriture à côté, omega se déplace vers la nourriture
                    orders = hungry0[0]
                    pos_omega_AB = hungry0[1] #nouvelle position de omega
                    if x_A_O > 1 or y_A_O > 1:
                        depla = move_wolf_to_wolf(pos_alpha_A, pos_omega_AB, orders)#comme un chien, alpha suit omega (mais un loup n'est-il pas un chien?)
                        orders = depla[0]

        if life_alpha <= 80 and ici == 'non':
            ...
            if can_use(orders, pos_alpha_A[0], pos_alpha_A[1]):

                hungry0 = move_food(pos_alpha_A, food, orders) #si pas de nourriture à proximité, alpha se rapproche de la plus proche
                orders = hungry0[0]
                pos_alpha_AB = hungry0[1]

                if x_A_O > 1 or y_A_O > 1:
                    depla = move_wolf_to_wolf(pos_omega_A, pos_alpha_AB, orders)#omega suit, il faut rester à côté de alpha pour le protéger après tout!
                    orders = depla[0]
            else:
                None
        else:
    ...
    for wolvesA in player_A:
        ...
        if player_A[wolvesA]['type'] != 'alpha' and player_A[wolvesA]['type'] != 'omega':
            ...
            #Cas où les loups nomaux ont plus de 20 hp
                ...
                #pas oublier de faire manger si plus de 80 hp, j'ai juste la flemme de le faire mtn

                #Si le omega adverse à plus de 25 hp, on se dirige vers lui pour l'attaquer en priorité
                if player_B[omega_B]['life'] >= 25:
                    tuple44 = move_wolf_to_wolf(wolvesA, omega_B, orders)
                    orders = tuple44[0]
                #Si le omega adverse à moins de 25 hp, on se dirige vers alpha pour l'attaquer
                else:
                    tuple22 = move_wolf_to_wolf(wolvesA, alpha_B, orders)
                    orders = tuple22[0]
            ...
            #Cas où nos loups ont moins de 20 hp
                ...
                if can_use(orders, wolvesA[0], wolvesA[1]):
                    #Sinon, ils se déplacent vers la nourriture
                    hungry0 = move_food(wolvesA, food, orders)
                    orders = hungry0[0]
    return orders
